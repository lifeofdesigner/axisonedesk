...
**Phase 3b, slice 2 — full profile fields + module-defaults application + audit log (done, 2026-08-25)**
- `supabase/migrations/0032_onboarding_full_profile.sql`: `create_organization_with_owner()` now persists company_size/employee_count/branch_count/warehouse_count/country/timezone/currency/preferred_language; applies the selected `organization_type`'s default module set as `org_feature_flags` rows (registry-driven, verified live for `retail`); logs an `organization.created` audit event (previously never logged).
- `supabase/migrations/0033_fix_create_organization_overload_2.sql`: corrective migration — same overload bug as `0030` recurred (adding parameters via `create or replace function` again created a duplicate overload instead of replacing), caught and fixed live before it could cause an issue.
- `OnboardingForm.tsx` extended to collect all 8 new fields, rendered only when `onboarding.industry_registry_picker` is on (flag stays OFF by default — legacy flow is byte-identical to before).
- **Not built, deliberately**: "Workspace" as a separate entity (doesn't exist — `organizations` already is it), "Default Departments" (no such table/concept exists), "Dashboard Configuration" (no `organization_type_dashboard_config` table was ever built, only proposed), "AI Configuration" (no live AI system exists to configure) — see ADR-010, `docs/00_ADOS/DECISIONS.md`, for full reasoning on each.

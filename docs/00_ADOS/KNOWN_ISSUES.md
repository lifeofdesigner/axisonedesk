...
| Migrations `0026_module_registry.sql` and `0027_industry_registry.sql` not applied to any live database | Database | Medium | Written 2026-08-25 (Industry Engine Phases 1-2); this environment has no authenticated Supabase CLI access to the actual linked project — re-verified via `supabase link`, which fails with a privileges error. See [docs/00_ADOS/NEXT_TASK.md](NEXT_TASK.md) for the resolution steps. `database.types.ts` additions for `modules`/`organization_types`/`organization_type_modules` were hand-authored pending CLI regeneration. |

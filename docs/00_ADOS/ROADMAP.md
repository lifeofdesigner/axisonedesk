...
- **Current status**: **Phase 1 (Module Registry) and Phase 2 (Industry/Org-Type Registry + Templates) — both live.** Phase 1: `modules` table, seed data (12 real modules), `platform_list_modules()`/`platform_upsert_module()` RPCs, `src/core/modules/{api.ts,hooks.ts}` (`supabase/migrations/0026_module_registry.sql`). Phase 2: `organization_types` + `organization_type_modules` tables, 14 seeded industries (module mappings for the 9 with a researched default — see [docs/18_REFERENCE/INDUSTRY_REGISTRY.md](../18_REFERENCE/INDUSTRY_REGISTRY.md)), `platform_list_organization_types()`/`platform_upsert_organization_type()`/`platform_archive_organization_type()`/`platform_restore_organization_type()`/`platform_set_organization_type_module()` RPCs, `src/core/industries/{api.ts,hooks.ts}` (`supabase/migrations/0027_industry_registry.sql`). Both migrations applied to the live "Axis" project 2026-08-25 (CLI access was restored mid-session); `database.types.ts` regenerated via CLI and verified against the hand-authored version (identical content, one real correction: RPC scalar `Args` are non-nullable `string`) — see [docs/18_REFERENCE/MODULE_REGISTRY.md](../18_REFERENCE/MODULE_REGISTRY.md) for full verification detail. `src/router.tsx` and `organizations` intentionally still untouched — see ADR-005 and ADR-006 in [DECISIONS.md](DECISIONS.md), which remain valid scoping decisions independent of DB access. Per the Incremental Delivery Rule in [AI_INSTRUCTIONS.md](AI_INSTRUCTIONS.md), Phase 3 requires a fresh "Continue" instruction.
- **Phases** (see playbook for detail): 1) Module Registry — **live**, 2) Industry/Org-Type Registry + Templates — **live**, 3) Onboarding wizard rewrite to apply templates, 4) Platform Owner Portal "Industries" management UI + Navigation/Dashboard generation from registry, 5) AI behavior selection by industry, 6) Subscription-gated module unlocking.

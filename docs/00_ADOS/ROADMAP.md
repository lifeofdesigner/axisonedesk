...
- **Current status**: **Phases 1, 2, 3a, 3b (both slices), 4-slice-1 live.** Phase 3b: full onboarding field collection, registry-driven module-defaults application, audit logging (`0032`) — flag `onboarding.industry_registry_picker` remains OFF pending human browser QA (ADR-010). Phase 4 slice 1: Dynamic Experience Engine foundation — `organization_types.experience_config` jsonb (`0034_experience_config.sql`), seeded with real KPI-definition/quick-action/empty-state content for 6 industries (restaurant, hotel, retail, wholesale, manufacturing, healthcare) from concrete user examples, the other 8 intentionally `null`. **Wired into real, working UI**: Dashboard renders a dynamic Quick Actions row, Inventory's empty state uses configured copy when present — both live for the 3 existing `retail`-type organizations. Six of the nine requested "engines" (KPI value computation, dashboard layout/widgets, Reports, Search, AI Experience, Demo Data) were explicitly NOT built because the underlying systems they'd need don't exist — full per-item reasoning in ADR-011, [DECISIONS.md](DECISIONS.md). Two real bugs (duplicate function overloads) were caught and fixed live this session, on top of the two from the prior session.
- **Phases** (see playbook for detail): 1) Module Registry — **live**, 2) Industry/Org-Type Registry + Templates — **live**, 3a) `organizations` schema extension — **live**, 3b) Onboarding rewrite — **live, flag OFF pending human QA**, 4 slice 1) Dynamic Experience Engine foundation (Quick Actions, Empty States) — **live**, 4 slice 2) Navigation generation from Module Registry — not started, 4 slice 3+) KPI values, Reports Engine, Search Engine, Demo Data, Onboarding tasks, Platform Owner config UI — not started, 5) AI behavior selection by industry (blocked on live AI system existing at all), 6) Subscription-gated module unlocking.

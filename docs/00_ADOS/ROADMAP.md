...
- **Current status**: **Phases 1, 2, 3a, 3b (both slices) live.** Full field collection (company size, employees, branches, warehouses, country, timezone, currency, language), registry-driven module-defaults application, and audit logging all ship in `0032_onboarding_full_profile.sql`, gated behind the same `onboarding.industry_registry_picker` flag as slice 1. **The flag remains OFF** — a full live end-to-end signup verification requires a human clicking through the actual browser flow, which this environment cannot safely perform (see ADR-010, [DECISIONS.md](DECISIONS.md)). Everything that *can* be verified without live signup was: migrations apply cleanly, the module-defaults join produces exactly the expected `org_feature_flags` rows for a real registry key, full compile/typecheck passes. Two real bugs were caught and fixed live this session (both times: `create or replace function` with added parameters silently creating a duplicate overload instead of replacing — `0030` and `0033`). "Workspace," "Default Departments," "Dashboard Configuration," and "AI Configuration" were explicitly NOT built — none of those systems exist yet, and fabricating them was rejected per the no-fabrication rule (full reasoning per item in ADR-010). `src/router.tsx`, navigation generation, and the cosmetic nav-label components remain untouched, per current instruction and Phase 4's not-started status.
- **Phases** (see playbook for detail): 1) Module Registry — **live**, 2) Industry/Org-Type Registry + Templates — **live**, 3a) `organizations` schema extension — **live**, 3b) Onboarding rewrite (both slices: canonical Source of Truth, full field collection, module-defaults application, audit logging) — **live, flag OFF pending human QA**, 4) Platform Owner Portal "Industries" management UI + Navigation/Dashboard generation from registry, 5) AI behavior selection by industry, 6) Subscription-gated module unlocking.

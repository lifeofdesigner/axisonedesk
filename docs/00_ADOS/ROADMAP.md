...
- **Current status**: **Phase 1 and Phase 2 live. Phase 3a (schema) live; Phase 3b (onboarding rewrite) not started.** Phase 1: `modules` table, seed data (12 real modules), `platform_list_modules()`/`platform_upsert_module()` RPCs, `src/core/modules/{api.ts,hooks.ts}` (`0026_module_registry.sql`). Phase 2: `organization_types` + `organization_type_modules` tables, 14 seeded industries, RPCs, `src/core/industries/{api.ts,hooks.ts}` (`0027_industry_registry.sql`). Phase 3a: `organization_type_key` + 6 other nullable columns added to `organizations` (`0028_organization_type_columns.sql`) — schema only, zero behavior change; discovered along the way that `organizations.business_type` already exists as an uncontrolled free-text field collected by onboarding today, unreconciled with the new `organization_type_key` (see ADR-008). All three migrations applied to the live "Axis" project and verified by direct query. `src/router.tsx` and the actual `/onboarding` flow remain untouched — see ADR-005, ADR-006, ADR-008 in [DECISIONS.md](DECISIONS.md). Per the Incremental Delivery Rule in [AI_INSTRUCTIONS.md](AI_INSTRUCTIONS.md), Phase 3b requires a fresh "Continue" instruction.
- **Phases** (see playbook for detail): 1) Module Registry — **live**, 2) Industry/Org-Type Registry + Templates — **live**, 3a) `organizations` schema extension — **live**, 3b) Onboarding wizard rewrite (incl. `business_type` reconciliation) — not started, 4) Platform Owner Portal "Industries" management UI + Navigation/Dashboard generation from registry, 5) AI behavior selection by industry, 6) Subscription-gated module unlocking.

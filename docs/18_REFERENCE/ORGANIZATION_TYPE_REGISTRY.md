...
**Live as of 2026-08-25** (`organization_types` + `organization_type_modules`, `supabase/migrations/0027_industry_registry.sql`) — see [INDUSTRY_REGISTRY.md](INDUSTRY_REGISTRY.md) for full status and verification detail. "Organization Type" and "Industry" share the same table — this registry is not a separate system. 14 system-default types are seeded; the Platform Owner Portal UI for creating additional types without code changes doesn't exist yet (Phase 4).

...
| 0034 | `0034_experience_config.sql` | Adds `experience_config jsonb` to `organization_types` — Dynamic Experience Engine foundation, Industry Engine Phase 4 slice 1 (see ADR-011, [docs/00_ADOS/DECISIONS.md](../00_ADOS/DECISIONS.md)). Seeded with real KPI-definition/quick-action/empty-state content for 6 industries (restaurant, hotel, retail, wholesale, manufacturing, healthcare) taken directly from user-provided examples; the other 8 industries intentionally left `null` — no fabricated defaults. **Applied to the live "Axis" project 2026-08-25**, verified via direct query (`retail`'s config spot-checked, matches seed exactly). |
